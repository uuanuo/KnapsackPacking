...
def get_constant_score(item, value_weight=VALUE_WEIGHT, area_weight=AREA_WEIGHT, can_use_constant_score=CAN_USE_CONSTANT_SCORE):
...
def get_item_profitability_ratio(item, area_weight=AREA_WEIGHT):
...
def get_weighted_sum_of_item_value_and_profitability_ratio(item, value_weight=VALUE_WEIGHT, area_weight=AREA_WEIGHT):
    ...
    for _ in range(repetition_num):
        ...
        if problem.container.max_weight and items_by_weight:
            ...
            for i in range(max_iter_num):
                ...
                if item_index_to_place_first >= 0 and i < max_iter_num * 0.1:
